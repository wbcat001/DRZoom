...
import { NearestPointsList } from './NearestPointsList';
import type { NearestPoint } from './NearestPointsList';
...
const FetchSelectedData = async (fileName: string) => {
    const response = await fetch(`http://localhost:8000/init/${fileName}`, {
        method: "POST",
        headers: {
            'Content-Type': 'application/json',
        }
...
const FetchClickData = async (point: ScatterPoint) => {
    const index = point.index;
    const response = await fetch(`http://localhost:8000/select/${index}`, {
        method: "POST",
        headers: {
            'Content-Type': 'application/json',
        }
...
export const DashBoard = () => {
    ...
    const handleFileSelect = (value: string) => {
        ...
        FetchSelectedData(value).then(fetchedData => {
            setData(fetchedData);
        });
    }

    // Click Point
    const handleClickPoint = (selectedPoint: ScatterPoint) => {
        setSelectedPoint(selectedPoint)
        FetchClickData(selectedPoint).then(fetchedNearestPoints => {
            setNearestPoints(fetchedNearestPoints.closest_points);

            console.log("nearest points", fetchedNearestPoints.closest_points);
        });
    }
        ...
        <div className="grid grid-cols-12 gap-2 h-[calc(100vh-120px)]">
          ...
          <div className="col-span-8">
            <div className="bg-white rounded-lg shadow h-full">
              ...
              <div className="p-6 h-[calc(100%-80px)]">
                ...
                selectedIndices={nearestPoints.map(point => point.index)}  // Example: all points are selected
                />
              </div>
            </div>
          </div>
          ...
          <div className="col-span-2 space-y-6">
            <div>
                ...
                <NearestPointsList points={nearestPoints} />
            </div>
            {/* Analysis Charts */}
            <div className="bg-white rounded-lg shadow p-6">
