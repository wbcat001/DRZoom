...
class DataManager(BaseDataManager):
    ...
    def load(self) -> Data:
        high_dim_data = self.load_high_dimensional_data(
            os.path.join(self.dir_path, "high_dim_data.pkl")
        )
    ...
    def load_high_dimensional_data(self, file_path: str) -> HighDimensionalData:
        ...
        try:
            ...
            window = 50
            windowed_data = np.zeros(
                (high_dim_data.shape[0] - window, high_dim_data.shape[1])
            )
