...
class LayoutStateRepository(Repository[Tuple[PositionData, List[int]]]):
    ...
    def set_data(self, data: Tuple[PositionData, List[int]], id:str) -> None:
        self.cache = data

    def get_data(self, id:str) -> Tuple[PositionData, List[int]]:
...
class ConfigRepository(Repository[Config]):
    ...
    def set_data(self, data: Config, id:str) -> None:
        self.cache = data

    def get_data(self, id: str) -> Config:
        if self.cache is None:
            raise ValueError("Cache is not set yet.")
        return self.cache
