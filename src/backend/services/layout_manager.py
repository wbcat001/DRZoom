...
class LayoutManager(BaseLayoutManager[PositionData]):
    ...
    def init_layout(
        ...
        layout = process_manager.process(
            data=high_dim_data, prev_layout=None, config=config
        )
    ...
    def update_layout(
        ...
        prev_filtered_position_data = self.filter_manager.filter_position_data(
            common_indecies, prev_layout
        )
        filtered_high_dim_data = self.filter_manager.filter_high_dim_data(
            common_indecies, high_dim_data
        )

        # レイアウトの計算
        new_layout = process_manager.process(
            filtered_high_dim_data, prev_filtered_position_data, config=config
        )
