...
class D3DataManager:
    ...
    def __init__(self):
        ...
        self.base_path = Path(__file__).parent.parent.parent / "experiments"
    ...
    def _load_configuration(self):
        ...
        #   - data.npz: embedding (N,2), words (N,)
        #   - condensed_tree_object.pkl: HDBSCAN condensed tree
        #   - cluster_to_label.csv: cluster labels (optional)
        # ============================================================
        self.datasets_config = {
            "default": {
                "name": "RAPIDS HDBSCAN Result",
                "description": "HDBSCAN clustering result from RAPIDS GPU acceleration",
                "data_path": "18_rapids/result/20251203_053328",  # ← CHANGE THIS PATH
                "point_count": 100000,
        ...
        self.use_mock_data = True  # ← False にすると実データを読み込み
    ...
    def get_initial_data(
        ...
        # Load embeddings
        data_file = data_path / "data.npz"
        
        # ============================================================
        # If data files don't exist, use mock data for development
        # ============================================================
        if not data_file.exists() or self.use_mock_data:
        ...
        # Load cluster metadata (stability, Strahler number, size)
        ...
        point_cluster_map = self._create_point_cluster_map(hdbscan_condensed_tree, point_count)
        
        # Update cluster IDs in points
        for i, point in enumerate(points):
            point["c"] = point_cluster_map.get(i, -1)
    ...
    def _load_cluster_labels(self, label_file: Path) -> Tuple[Dict[int, str], Dict[int, List[str]]]:
        """
        Load cluster labels and representative words
        """
        ...
        try:
            ...
            with open(label_file, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    cluster_words[cluster_id] = [w.strip() for w in words]
        except Exception as e:
