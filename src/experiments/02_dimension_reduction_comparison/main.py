...
# 次元削減アルゴリズムのインポート
from src.experiments.dimensionality_reduction.pca import PCA
from src.experiments.dimensionality_reduction.tsne import TSNE
from src.experiments.dimensionality_reduction.umap import UMAP
...
class DimensionReductionExperiment:
    ...
    def run_all_algorithms(self):
        ...
        # 独自実装のアルゴリズムをインスタンス化
        algorithms = {
            'PCA': PCA(self.output_dim, self.random_state),
            't-SNE': TSNE(self.output_dim, random_state=self.random_state),
            'UMAP': UMAP(self.output_dim, random_state=self.random_state)
        }
        
        # 追加のscikit-learnアルゴリズム
        ...
        # 全アルゴリズムを結合
        algorithms.update(sklearn_algorithms)
