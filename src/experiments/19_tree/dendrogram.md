...
以下はノートブック内でダッシュアプリが使う主要な変数／データ構造と、その作成・計算方法の要約です。実装の細部は `dendrogram.ipynb` の該当セルに従いますが、ここでは読みやすく整理してあります。

- node_id_map (リンク行列のID再マッピング辞書)
    - 目的: フィルタ後のリンケージ（Z行列）で元のノードID（HDBSCANや元ZのID）を連続した小さなIDに変換する。
    - 計算方法:
        1. フィルタされたZ行列（StrahlerやStabilityで絞ったもの）から葉ノードを取得する（get_leaves や同等ロジック）。
        2. 葉を順に current_id = 0 から割り当てて node_id_map[leaf] = current_id とする。
        3. フィルタ内に現れる親ノード(parent)も順次 current_id を進めて割り当てる。
    - 形状: dict{ int(original_node_id) -> int(compact_id) }

- point_id_map (データポイント -> クラスタID マッピング)
    - 目的: 散布図上の点（観測インデックス）から、その点が属するクラスタID（HDBSCANのクラスタID）を素早く取得する。
    - 計算方法:
        1. condensed_tree._raw_tree（pandas構造）から葉に相当する行（child が観測点）を抽出。
        2. { int(row['child']) : int(row['parent']) } というdict内包表現でマップを作る。
    - 形状: dict{ int(point_index) -> int(cluster_node_id) }

- selected_cluster_ids / selected_new_cluster_ids
    - 目的: 散布図で選択された点群から、デンドログラム上で強調すべきノード（クラスタ）ID群を得る。
    - 計算方法:
        1. `selectedData['points']` の pointIndex（観測インデックス）を取り出す。
        2. `get_clusters_from_points(point_ids, point_id_map)` を呼び、対応するクラスタID集合を取得（内部で set を使って重複除去）。
        3. フィルタ済みZに対応するIDへは `[_node_id_map[k] for k in selected_cluster_ids if k in _node_id_map]` のようにマップ変換する。

- is_selected / is_selecteds (強調用の論理配列)
    - 目的: Plotly の trace（セグメントごと）に対して、強調表示（opacityやhover可否）を適用する。
    - 計算方法:
        1. 選択クラスタID（フィルタ後ID）を3倍展開して、各マージ（Zの行）に対応する3つのセグメントにマップする（`sum([[i]*3 for i in selected_ids], [])` のパターン）。
        2. 元のZ行列に対して `is_selected = [True if row[2] in selected_new_cluster_ids else False for row in Z]` のように作成し、さらに各要素を3倍展開してセグメント単位の配列に揃える。
    - 形状: list[bool] 長さはセグメント数 (通常 3 * (n_merges))

- nodes / leaf_to_x / leaf_order (デンドログラム座標計算の中間構造)
    - nodes: 各ノード（葉・内部）の辞書リスト。各要素はキー { 'x', 'y', 'size', 'left', 'right' } を持つ。
        - 葉ノードは x=None, y=0, size=1
        - 内部ノードは Z 行から y=distance, size=count, left=c1, right=c2 をセット
    - leaf_order: 再帰的に左右をたどって得る葉の順序。実装では子ノードのサイズ（`size`）を比較して大きい方を左にするソートが使われている（見た目の安定化目的）。
    - leaf_to_x: leaf_order を 1,3,5,... の奇数に割り当てる辞書 { leaf_idx: x_coord }
    - 計算: ルートから再帰的に x を計算し、内部ノードの x は子の平均をとる。

- icoord / dcoord / segments (描画用座標)
    - icoord: 各マージについて [x1, x2, x3, x4] というX座標配列（U字の4点）を格納したリスト。
    - dcoord: 各マージについて対応するY座標 [y1, y2, y3, y4] を格納したリスト。
    - segments: `get_dendrogram_segments2` で icoord/dcoord を3本の線分に分解し、最終的に描画用に [(x1,y1),(x2,y2)], [(x2,y2),(x3,y3)], [(x4,y4),(x3,y3)] のようなタプル列にして返す。

- hover_lines / full_hover_text (ホバーテキスト)
    - 目的: デンドログラムの各セグメントに表示する補助情報（スコア、追加メタデータなど）。
    - 計算方法（ノートブック実装の要点）:
        1. `expanded_scores`（各セグメントあるいはマージごとのスコア配列）が渡されている場合、対応するインデックスから score_value を取得し、`Score: {score_value:.4f}` という行を `hover_lines` に追加する。
        2. `additional_data`（kwargs で渡される `(key, expanded_value_list)` のリスト）の各要素について、対応する値を取り出し、数値なら小数形式で、非数値はそのまま文字列として `hover_lines` に追加する。
        3. 最後に `'<br>'.join(hover_lines)` で `full_hover_text` を作成し、`text=[full_hover_text] * len(x_coords)` のように trace に供給する（各点に同じテキストを充てる）。
    - 補足: 選択されていないセグメントには `hoverinfo='skip'` を設定してホバーを無効化している。

- expanded_is_selecteds / expanded_scores（内部展開版配列）
    - 目的: 元の Z 行（1行）が生成する 3 本の線分に対応する「展開後の配列」を扱いやすく持つための配列。実装では Z 行を3つに展開した長さの arrays を作ることが多い。
    - 例: `expanded_is_selecteds[i]` が False の場合、そのセグメント群は opacity=0.2, hoverinfo='skip' となる。

注意・仮定:
- ノートブックは複数の実験的実装が混在しており、`point_id_map` や `node_id_map` を作る箇所が複数存在します（同等のロジックが別セルで繰り返し実装されていることがあります）。ここでは最も直近で参照される実装（`filter_linkage_matrix_by_strahler` 内のマッピングや `condensed_tree._raw_tree` を基にした `point_id_map`）に基づいて説明しました。
- もし別セルのバージョン（例えば `node_id_map` を先に作ってから別のフィルタ順序を通すもの）を正確に反映したい場合、どのセルを「正解」とするか教えてください。必要なら対象セル番号を明示してその箇所だけをドキュメント化します。

---

次のステップ: 要望があれば、これらの変数の「実際のサンプル出力（小さいZでの辞書中身や hover_text の例）」をノートに基づいて追加します。どれを優先でサンプル化しますか？
