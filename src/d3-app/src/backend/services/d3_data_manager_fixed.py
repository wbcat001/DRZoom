...
class D3DataManager:
    ...
    def get_initial_data(
        ...
        if hdbscan_file.exists():
            ...
            linkage_matrix = self._create_simplified_linkage_from_labels(point_cluster_labels)
        
        # Load cluster metadata
        if hdbscan_condensed_tree is not None:
            cluster_meta = self._compute_cluster_metadata(hdbscan_condensed_tree)
        else:
            cluster_meta = self._compute_cluster_metadata_from_labels(point_cluster_labels)
