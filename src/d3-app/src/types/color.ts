 ...
 * 3. Dendrogram selection (lime green)
 * 4. Default (blue)
 * 5. Dimmed (light blue)
 */
...
export const HIGHLIGHT_COLORS: Record<HighlightType, string> = {
  ...
  heatmap_to_dr: '#FF1493',     // Deep pink - heatmap-to-DR highlighting
  dendrogram_to_dr: '#32CD32'   // Lime green - dendrogram-to-DR highlighting
};
 ...
 * Follows the priority order: heatmap > dr > dendrogram > default
 */
...
export function getHighlightOpacity(state: HighlightState, anySelectionActive: boolean): number {
  if (state.isDRSelected || state.isHeatmapClicked || state.isDendrogramSelected) {
    return 1.0;
  }
